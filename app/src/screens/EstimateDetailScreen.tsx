...
import { useApp, Estimate, EstimateStatus } from '../context/AppContext';
...
function buildPdfHtml(estimate: Estimate, projectName: string, clientName: string, address: string, serviceType: string, docType: 'Estimate' | 'Invoice', invoiceNumber?: string): string {
  ...
  return `
    ...
    <body>
      ...
      <div class="footer">
        Generated by PhotoQuote AI &bull; ${docType === 'Invoice' ? 'Payment due within 30 days' : 'Valid for 30 days'} &bull; ${new Date(estimate.createdAt).toLocaleDateString()}
      </div>
    </body>
    </html>
  `;
...
export default function EstimateDetailScreen({ navigation, route }: EstimateDetailScreenProps) {
  const { estimates, getProject, getClient, updateEstimate, addInvoice, getEstimateInvoice } = useApp();
  ...
  const handleGeneratePdf = async (docType: 'Estimate' | 'Invoice' = 'Estimate') => {
    try {
      const html = buildPdfHtml(estimate, projectName, clientName, address, serviceType, docType, invoice?.invoiceNumber);
  ...
  const sendVia = async (method: 'email' | 'sms' | 'whatsapp', docType: 'Estimate' | 'Invoice' = 'Estimate') => {
    const text = buildShareText(estimate, projectName, clientName, address, serviceType, docType, invoice?.invoiceNumber);
    const plainText = text.replace(/\*/g, '');

    try {
      if (method === 'email') {
        const subject = encodeURIComponent(`${docType} - ${projectName}`);
      ...
      // Auto-advance status from Draft to Sent when sending estimate
      if (docType === 'Estimate') {
        autoAdvanceToSent();
      }
    } catch {
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {/* Action Buttons: PDF + Invoice side by side */}
        <View style={styles.actionRow}>
          <TouchableOpacity style={styles.actionButton} onPress={() => handleGeneratePdf('Estimate')}>
            <Text style={styles.actionIcon}>PDF</Text>
            <Text style={styles.actionText}>Estimate PDF</Text>
          </TouchableOpacity>
        ...
        {/* Send Estimate */}
        <View style={styles.card}>
          <Text style={styles.cardTitle}>Send Estimate</Text>
          <TouchableOpacity style={styles.sendButton} onPress={() => sendVia('email', 'Estimate')}>
          ...
          <TouchableOpacity style={styles.sendButton} onPress={() => sendVia('sms', 'Estimate')}>
            <Text style={styles.sendIconText}>💬</Text>
            <View style={styles.sendInfo}>
          ...
          <TouchableOpacity style={[styles.sendButton, styles.sendButtonWhatsApp]} onPress={() => sendVia('whatsapp', 'Estimate')}>
            <Text style={styles.sendIconText}>📱</Text>
            <View style={styles.sendInfo}>
