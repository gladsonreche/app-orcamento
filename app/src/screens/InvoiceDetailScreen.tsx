...
import { useApp, Invoice, InvoiceStatus } from '../context/AppContext';
...
function buildInvoicePdf(invoice: Invoice, projectName: string, clientName: string, address: string, serviceType: string): string {
  ...
  return `
    ...
    <body>
      ...
      <div class="footer">
        Generated by PhotoQuote AI &bull; Invoice #${invoice.invoiceNumber} &bull; ${new Date(invoice.createdAt).toLocaleDateString()}
      </div>
    </body>
    </html>
  `;
...
export default function InvoiceDetailScreen({ navigation, route }: InvoiceDetailScreenProps) {
  const { invoices, getProject, getClient, updateInvoice } = useApp();
  ...
  const handleGeneratePdf = async () => {
    try {
      const html = buildInvoicePdf(invoice, projectName, clientName, address, serviceType);
  ...
  const sendVia = async (method: 'email' | 'sms' | 'whatsapp') => {
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {/* Send Invoice */}
        <View style={styles.card}>
          <Text style={styles.cardTitle}>Send Invoice</Text>
          <TouchableOpacity style={styles.sendButton} onPress={() => sendVia('email')}>
            <Text style={styles.sendIconText}>✉</Text>
            <View style={styles.sendInfo}>
              <Text style={styles.sendLabel}>Email</Text>
              <Text style={styles.sendDetail}>{clientEmail || 'No email on file'}</Text>
            </View>
          </TouchableOpacity>

          <TouchableOpacity style={styles.sendButton} onPress={() => sendVia('sms')}>
            <Text style={styles.sendIconText}>💬</Text>
            <View style={styles.sendInfo}>
              <Text style={styles.sendLabel}>SMS</Text>
              <Text style={styles.sendDetail}>{clientPhone || 'No phone on file'}</Text>
            </View>
          </TouchableOpacity>

          <TouchableOpacity style={[styles.sendButton, styles.sendButtonWhatsApp]} onPress={() => sendVia('whatsapp')}>
            <Text style={styles.sendIconText}>📱</Text>
            <View style={styles.sendInfo}>
              <Text style={[styles.sendLabel, { color: '#25d366' }]}>WhatsApp</Text>
              <Text style={styles.sendDetail}>{clientPhone || 'No phone on file'}</Text>
            </View>
          </TouchableOpacity>
        </View>
...
const styles = StyleSheet.create({
  ...
  // PDF
  pdfButton: { backgroundColor: '#1a73e8', borderRadius: 12, padding: 18, alignItems: 'center', marginBottom: 16 },
  pdfButtonText: { color: '#fff', fontSize: 18, fontWeight: '600' },

  // Send buttons
  sendButton: {
    flexDirection: 'row', alignItems: 'center', padding: 14, borderRadius: 12,
    borderWidth: 1, borderColor: '#e0e0e0', marginBottom: 10, backgroundColor: '#f8f9fa',
  },
  sendButtonWhatsApp: { borderColor: '#25d366' },
  sendIconText: { fontSize: 28, marginRight: 14 },
  sendInfo: { flex: 1 },
  sendLabel: { fontSize: 16, fontWeight: '600', color: '#333', marginBottom: 2 },
  sendDetail: { fontSize: 13, color: '#666' },
});
